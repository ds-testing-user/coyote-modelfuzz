...
using Microsoft.Coyote.Samples.CloudMessaging.Specifications;
...
namespace Microsoft.Coyote.Samples.CloudMessaging.Mocks
{
    ...
    public class MockServerHost : IServerManager
    {
        ...
        /// </summary>
        public TimeSpan LeaderElectionDueTime => TimeSpan.FromSeconds(10); // TODO: randomize 10-20

        /// <summary>
        /// The leader election periodic time interval.
        /// </summary>
        public TimeSpan LeaderElectionPeriod => this.GetRandomTimeSpan(50, 100); // TODO: randomize 10-20
        ...
        private TimeSpan GetRandomTimeSpan(int min, int max)
        {
            int rand;
            do
            {
                rand = this.Runtime.RandomInteger(max);

                if (rand < min)
                {
                    Console.WriteLine($"Chosen integer is smaller than given minimum({min}). Choosing again.");
                }
            }
            while (rand < min);
