    ...
    public class Server : StateMachine
    {
        ...
        private void AppendLogEntriesRequest(Event e)
        {
            ...
            if (appendEntries)
            {
                if (request.Term < this.CurrentTerm)
                {
                    this.SendEvent(this.ClusterManager, new AppendLogEntriesResponseEvent(request.LeaderId, this.Manager.ServerId, this.CurrentTerm, false, request.LeaderCommit, request.Command)); // QA: What to do if term is higher than the request?
                ...
                else
                {
                    ...
                    {
                        this.SendEvent(this.ClusterManager, new AppendLogEntriesResponseEvent(request.LeaderId, this.Manager.ServerId, this.CurrentTerm, false, request.LeaderCommit, request.Command));
                        this.Logger.WriteLine($"<AppendLogEntriesResponse> {this.Manager.ServerId} sent append " +
                            $"entries response (term={this.CurrentTerm}, log={this.Logs.Count}, " +
                    ...
                    else
                    {
                        ...
                        this.SendEvent(this.ClusterManager, new AppendLogEntriesResponseEvent(request.LeaderId, this.Manager.ServerId, this.CurrentTerm, true, request.LeaderCommit, request.Command)); // QA: this.CommitIndex?
        ...
        {
            Console.WriteLine($"Leader-{this.Id} received client request.");
