...
namespace Microsoft.Coyote.SystematicTesting
{
    ...
    public sealed class TestingEngine : IDisposable
    {
        ...
        /// </summary>
        internal ActorOperationScheduler Scheduler { get; private set; }
        ...
        /// </summary>
        internal List<List<string>> Actions { get; private set; }
        ...
        private TestingEngine(Configuration configuration, TestMethodInfo testMethodInfo, ExecutionTrace prefixTrace, LogWriter logWriter)
        {
            ...
            this.Scheduler = ActorOperationScheduler.Setup(configuration, prefixTrace);
        ...
        private Task CreateTestingTask(TestMethodInfo methodInfo)
        {
            return new Task(() =>
            {
                ...
                try
                {
                    ...
                    // Run the finalize methods of ActorOperationScheduler
                    this.Scheduler.Finalize(this.LogWriter);
        ...
        private bool RunNextIteration(TestMethodInfo methodInfo, uint iteration)
        {
            ...
            // TODO: Pass actor execution Trace if the scheduler supports is
            ...
            try
            {
                ...
                // Save the actions taken in this iteration
                this.Scheduler.ActorExecutionTrace = actorRuntimeExtension.Trace;
                ...
                this.GatherTestingStatistics(runtime, iteration);
                    ...
                    runtime.ExecutionStatus is ExecutionStatus.Deadlocked))
                {
                    ...
                    this.Scheduler = ActorOperationScheduler.Setup(this.Configuration, SchedulingPolicy.Fuzzing, this.Scheduler.ValueGenerator);
            ...
            finally
            {
                ...
                runtime?.Dispose();
                // TODO: Send the actions to TLC or the scheduler
            }
        ...
        /// </summary>
        private void GatherTestingStatistics(CoyoteRuntime runtime, uint iteration)
        {
            TestReport report = new TestReport(this.Configuration);
            runtime.PopulateTestReport(report, iteration);
