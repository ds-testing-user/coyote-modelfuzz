...
using QLearningTLAInterleavingStrategy = Microsoft.Coyote.Testing.Interleaving.QLearningStrategyTLA;
using RandomChoiceInterleavingStrategy = Microsoft.Coyote.Testing.Interleaving.RandomChoiceStrategy;
...
namespace Microsoft.Coyote.Runtime
{
    ...
    internal class OperationScheduler
    {
        ...
        /// </summary>
        protected readonly LinkedList<Strategy> Portfolio;

        /// <summary>
        /// The exploration strategy used in the current iteration.
        /// </summary>
        protected Strategy Strategy => this.Portfolio.First.Value;
        ...
        /// </summary>
        protected OperationScheduler(Configuration configuration, SchedulingPolicy policy, IRandomValueGenerator generator, ExecutionTrace prefixTrace)
        {
                ...
                if (this.SchedulingPolicy is SchedulingPolicy.Interleaving)
                {
                    switch (configuration.ExplorationStrategy)
                    {
                        ...
                        case ExplorationStrategy.QLearning:
                            ...
                            break;
                        case ExplorationStrategy.QLearningTLA:
                            this.Portfolio.AddLast(new QLearningTLAInterleavingStrategy(configuration));
                            break;
                        case ExplorationStrategy.DFS:
                            this.Portfolio.AddLast(new DFSInterleavingStrategy(configuration));
                            break;
                        case ExplorationStrategy.RandomChoice:
                            this.Portfolio.AddLast(new RandomChoiceInterleavingStrategy(configuration));
                            break;
