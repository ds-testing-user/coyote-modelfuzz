...
namespace Microsoft.Coyote.Actors
{
    ...
    internal class ActorExecutionContext : IActorRuntime, IRuntimeExtension
    {
        ...
        /// </summary>
        internal ActorExecutionTrace Trace { get; }
        ...
        internal ActorExecutionContext(Configuration configuration, ActorLogManager logManager)
        {
            ...
            this.Trace = new ActorExecutionTrace();
        }
        ...
        private EnqueueStatus EnqueueEvent(ActorId targetId, Event e, Actor sender, EventGroup eventGroup, out Actor target)
        {
            ...
            this.Trace.Add(new SendEventStep(e, sender.Id, targetId));
        ...
        {
            bool choice = this.Runtime.GetNextNondeterministicBooleanChoice(callerName, callerType);
            this.Trace.Add(new NonDeterministicBooleanChoiceStep(choice));
            return choice;
        }
        ...
        {
            // TODO: add an action for non deterministic integer choice
            int choice = this.Runtime.GetNextNondeterministicIntegerChoice(maxValue, callerName, callerType);
            this.Trace.Add(new NonDeterministicIntegerChoiceStep(choice));
            return choice;
        }

        internal void LogGotoState(ActorId actor, string fromState, string toState)
        {
            this.Trace.Add(new StateTransitionStep(actor, fromState, toState));
            this.LogManager.LogGotoState(actor, fromState, toState);
        }
        ...
        {
            this.Trace.Add(new ActionInvokedStep(actor.Id, action.Name));
        ...
        internal virtual void LogDequeuedEvent(Actor actor, Event e, EventInfo eventInfo, bool isFreshDequeue)
        {
            ...
            this.Trace.Add(new ReceiveEventStep(e, eventInfo.OriginInfo.SenderActorId, actor.Id));
        ...
        {
            this.Trace.Add(new ActionInvokedStep(stateMachine.Id, action.Name));
        ...
        internal sealed class Mock : ActorExecutionContext
        {
            ...
            private EnqueueStatus EnqueueEvent(Actor actor, Event e, Actor sender, EventGroup eventGroup, SendOptions options)
            {
                ...
                this.Trace.Add(new SendEventStep(e, sender?.Id, actor.Id));
