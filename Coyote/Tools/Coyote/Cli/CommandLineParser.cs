...
namespace Microsoft.Coyote.Cli
{
    internal sealed class CommandLineParser
    {
        ...
        private Command CreateTestCommand(Configuration configuration)
        {
            ...
            var mutatorOption = new Option<string>(
                name: "--mutator",
                getDefaultValue: () => string.Empty,
                description: "Set the mutation strategy in the case of using interleaved-fuzzing")
            {
                ArgumentHelpName = "MUTATOR",
                Arity = ArgumentArity.ExactlyOne
            };
            ...
            this.AddOption(command, mutatorOption);
            ...
            if (!result.IsImplicit)
            {
                switch (result.Option.Name)
                {
                    ...
                    case "strategy":
                        ...
                        break;
                    case "mutator":
                        string mutator = result.GetValueOrDefault<string>();
                        this.Configuration.WithMutatorType(mutator);
                        break;
                    case "strategy-value":
